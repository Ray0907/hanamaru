...
test('buildDistribution writes minified ESM, IIFE, and exact CSS', async (t) => {
  ...
  await writeFile(path.join(root, 'src', 'hanamaru.css'), '.hana-fixture{}\n');
  ...
  assert.equal(await readFile(path.join(root, 'dist', 'hanamaru.css'), 'utf8'), '.hana-fixture{}\n');
});
